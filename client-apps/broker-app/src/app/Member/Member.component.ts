...
export class MemberComponent implements OnInit {
	...
	getForm(id: any): Promise<any> {
		...
		return this.serviceMember.getparticipant(id)
			...
			.then((result) => {
				...
				if (result.info && result.info.firstName) {
					formObject.firstName = result.info.firstName;
				} else {
					formObject.firstName = null;
				}

				if (result.info && result.info.lastName) {
					formObject.lastName = result.info.lastName;
				} else {
					formObject.lastName = null;
				}

				if (result.info && result.info.phone) {
					formObject.phone = result.info.phone;
				} else {
	...
	revokeID(id: any){
		return this.serviceMember.revokeID(id).toPromise()
			.then((res: any) => {
				console.log(res)
			}).catch((error) => {
				if (error === 'Server error') {
